...
class VendingMachineAPI:
    ...
    def update_machine_menu_items(self, machine_id: int, menu_item_ids: List[int], display_orders: List[int] = None, discount_rates: Optional[List[Optional[float]]] = None) -> Dict:
        """更新機台菜單項目（支援折扣率設定）"""
        api_logger.info(f"🔄 開始更新機台 {machine_id} 菜單項目")
        api_logger.info(f"📋 菜單項目ID: {menu_item_ids}")
        api_logger.info(f"📋 顯示順序: {display_orders}")
        api_logger.info(f"📋 折扣率: {discount_rates}")
        
        try:
            # 驗證 discount_rates 參數
            if discount_rates is not None:
                if len(discount_rates) != len(menu_item_ids):
                    api_logger.error(f"discount_rates 長度 ({len(discount_rates)}) 必須與 menu_item_ids 長度 ({len(menu_item_ids)}) 一致")
                    import streamlit as st
                    st.error("❌ 折扣率數量必須與菜單項目數量一致")
                    return None
                
                # 驗證每個折扣率值
                for i, rate in enumerate(discount_rates):
                    if rate is not None and (rate < 0.0 or rate > 1.0):
                        api_logger.error(f"discount_rates[{i}] = {rate} 必須在 0.0-1.0 範圍內")
                        import streamlit as st
                        st.error(f"❌ 折扣率[{i}] 必須在 0.0-1.0 之間")
                        return None
            ...
            if display_orders is not None:
                payload["display_orders"] = display_orders
            
            if discount_rates is not None:
                payload["discount_rates"] = discount_rates
